...
- [x] При запросе содержимого репозитория по названию ветки (или хэшу комита), для выполнения требования `отдать актуальное содержимое ветки` - выполняется `pull`  для обновления рабочей директории

- [x] Реализована очередь для роутов, где надо делать `pull` или `checkout`, или выполняется смена рабочей директории, для получения актуального содержания, иначе это может привести к ошибкам при выполнении разных асинхронных запросов
