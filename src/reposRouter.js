...
router.get('/', utils.wrapRoute(async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.status(501).json({ error: "An error occurred while trying to read the directory" });
    }
...
router.post('/:repositoryId', utils.wrapRoute(async (req, res) => {
    const { url } = req.body;
...
router.get(/^\/([^\/]+)(?:\/tree(?:\/([^\/]+)(\/.*)?)?)?$/, utils.wrapRoute(async (req, res) => {
